...
export default class BundleManager {
  constructor(configManager, fileSystemManager) {
  ...
  async generateAllBundles() {
    ...
    try {
      ...
      console.log('📊 Bundle generation complete, checking sizes before saving states...');
      const allBundles = this.configManager.getBundles();
      for (const [name, bundle] of allBundles) {
        console.log(`📦 Bundle "${name}": ${bundle.files?.length || 0} files, ${bundle.size || 0} bytes`);
        if (bundle.size > 10000000) { // 10MB
          console.warn(`⚠️ Large bundle detected: "${name}" is ${Math.round(bundle.size / 1024 / 1024)}MB`);
        }
      }
