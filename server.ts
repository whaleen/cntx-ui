...
export class CntxServer {
  ...
  semanticCache: any | null;
  lastSemanticAnalysis: number | null;
  vectorStoreInitialized: boolean;
  semanticAnalysisManager: any;
  activityManager: any;

  constructor(cwd = process.cwd(), options: ServerOptions = {}) {
    this.CWD = cwd;
  ...
  async getSemanticAnalysis() {
    // Try SQLite first
    try {
      const dbChunks = this.databaseManager.db.prepare('SELECT * FROM semantic_chunks').all() as any[];
  ...
  async enhanceSemanticChunksIfNeeded(analysis: any) {
  ...
  handleStaticFile(req: IncomingMessage, res: ServerResponse, url: any) {
...
export async function startServer(options: ServerOptions = {}) {
  ...
  if (options.withMcp !== false) server.startMCPServer();
